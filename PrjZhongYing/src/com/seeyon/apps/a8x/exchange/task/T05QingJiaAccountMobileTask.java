...
public final class T05QingJiaAccountMobileTask extends AbstrateExchangeTask {

	private static Log log = LogFactory.getLog(T05QingJiaAccountMobileTask.class);
	...
	public T05QingJiaAccountMobileTask(ExchangeTaskContext context) {
		super(context);
	...
	public void execute() {
		...
		writeWaitingExchageLog("YGQJ", "督导请假单", context.getFormTmpleteCode(), null, null, null, "QINGJIATASK");
		...
		log.info("[督导请假单]执行结束....,共耗时:" + times_);
	}
	...
	private void step2() {
		...
		if (billDate == null) {
			billDate = (java.util.Date) valuesMap.get("创建日期");
		}
		if (billDate == null) {
			logError("请假时间为空.");
			return;
	...
	public void end() {
		...
		// 记录执行日志
		if (this.isSuccess()) {
			writeExchageLog("Y", "YGQJ", "督导请假单", context.getFormTmpleteCode(), tmpFormDate, tmpFormCode, tmpFormInfo, "QINGJIATASK");
		} else {
			writeExchageLog("N", "YGQJ", "督导请假单", context.getFormTmpleteCode(), tmpFormDate, tmpFormCode, tmpFormInfo, "QINGJIATASK");
		}
		if (valuesMap != null) {
			valuesMap.clear();
			valuesMap = null;
