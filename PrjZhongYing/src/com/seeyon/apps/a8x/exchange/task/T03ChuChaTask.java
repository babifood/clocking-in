...
public final class T03ChuChaTask extends AbstrateExchangeTask {

	private static Log log = LogFactory.getLog(T03ChuChaTask.class);

	private Map<String, Object> valuesMap;

	private Map<String, Object> dataMap;

	public T03ChuChaTask(ExchangeTaskContext context) {
		super(context);
	 ...
	 */
	public void start() {
		writeWaitingExchageLog("YGCC", "出差申请单", context.getFormTmpleteCode(), null, null, null, "");
	}
	...
	public void execute() {
		...
		log.info("[出差申请单]执行结束....,共耗时:" + times_);
	}
	...
	private void step2() {
		...
		dataMap.put("业务单据类型", "出差申请单");

		java.util.Date billDate =  (java.util.Date) valuesMap.get("申请日期");
		if(billDate==null){
			billDate = (java.util.Date) valuesMap.get("实际出差时间左");
		}
		...
		dataMap.put("时长", valuesMap.get("实际出差时间共"));
		dataMap.put("开始时间", valuesMap.get("实际出差时间左"));
		dataMap.put("结束时间", valuesMap.get("实际出差时间右"));
	...
	public void end() {
		...
		// 记录执行日志
		if (this.isSuccess()) {
			writeExchageLog("Y", "YGCC", "出差申请单", context.getFormTmpleteCode(), tmpFormDate, tmpFormCode, tmpFormInfo, "");
		} else {
			writeExchageLog("N", "YGCC", "出差申请单", context.getFormTmpleteCode(), tmpFormDate, tmpFormCode, tmpFormInfo, "");
		}
		if (valuesMap != null) {
			valuesMap.clear();
			valuesMap = null;
